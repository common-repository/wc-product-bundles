				...
				if( $p->get_type() == "simple" ) {
					...
					foreach ( $childposts as $child ) {
						if( !in_array( $child->ID, $bundles ) ) {
							...
							$html .= '<li class="wcpb-product"><a href="#" product_id="'. $variation->get_id() .'" product_type="simple" title="'. esc_attr( trim( $title ) ) .'">#'. $variation->get_id() .' - '. esc_html( $product->post_title ) .' '. esc_html( trim( $title ) ) .'</a></li>';
						}						
					}
				}	
			}				
				...
				if( ( $bundles == null ) || in_array( $key, $newly_added ) ) {			
					...
					<div class="wcpb-product-bundle-row wc-metabox" product_id="<?php echo $key; ?>">
						<h3 class="wcpb-product-bundle-row-header">
							#<?php echo esc_html( $key ); ?> <?php echo esc_html( $p->get_title() );?> - <?php echo esc_html( $p->get_type() ); ?>
							<a href="#" title="<?php _e( 'Remove', 'wc-product-bundles' ); ?> <?php echo esc_attr( $p->get_title() ); ?> <?php _e( 'from bundle', 'wc-product-bundles' ); ?>" product_id="<?php echo esc_attr( $key ); ?>" class="button wcpb-remove-bundle-btn"><?php _e( 'Remove', 'wc-product-bundles' ); ?></a>
						</h3>
