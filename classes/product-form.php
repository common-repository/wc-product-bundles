...
class wcpb_product_form {
	...
	function __construct() {		
		...
		add_action( 'woocommerce_new_order_item', array( $this, 'wcpb_add_bundle_as_order_meta' ), 1, 3 );
	...
	function wcpb_product_form() { 
		...
		if( wcpb_utils::get_wcpb_meta( $product_id, '_wcpb_show_bundle_on_product', 'yes' ) == "yes" ) {		
	?>
	
		<div class="wcpb-bundled-products-container">
			<?php $bundles = apply_filters( 'wcpb/load/bundle', $product_id ); ?>
				...
				<table class="wcpb-bundled-product">
					<tr>
						...
						<td class="wcpb-thumbnail-td">				
							<a href="<?php echo esc_url( $product_url ); ?>" title="<?php echo esc_attr( $bundle->get_title() ); ?>" class="wcpb-featured"><?php echo $bundle->get_image( 'thumbnail' ); ?></a>					
						</td>
						<?php endif; ?>
						<!-- bundled product's summary section -->
						<td>
							<a href="<?php echo esc_url( $product_url ); ?>" class="wcpb-bundled-product-title"><h1><?php echo esc_html( $value['quantity'] ) ." x ". esc_html( $bundle->get_title() ); ?></h1></a>
							...
							<?php 
								...
								if( $value['desc'] != "" && $value['desc'] != null ) {
									...
									if( $bundle->get_short_description() != "" && $bundle->get_short_description() != null ) {
										$desc = $bundle->get_short_description();
									} else {
										$desc = wp_trim_words( $bundle->get_description(), apply_filters( 'excerpt_length', 55 ), "..." );
									}
								}							
							?>
					...
					if( $product_type == "wcpb" ) {
						...
						foreach ( $bundles as $key => $bundle ) {
							...
							if ( $_product && $_product->exists() && $_product->managing_stock() ) {
								$new_stock = wc_update_product_stock( $this, (intval( $item['qty'] ) * intval( $bundle['quantity'] )), 'decrease' );
				...
				if ( $item['product_id'] > 0 ) {
					...
					if( $product_type == "wcpb" ) {
						...
						foreach ( $bundles as $key => $bundle ) {
							$_product = wc_get_product( $key );
							if ( $_product && $_product->exists() && $_product->managing_stock() ) {
								$new_stock = wc_update_product_stock( $this, (intval( $item['qty'] ) * intval( $bundle['quantity'] )), 'increase' );
