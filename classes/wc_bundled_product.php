			...
			if( is_array( $bundles ) ) {
				foreach ( $bundles as $key => $value ) {
					...
					if( $value["tax_included"] == "yes" ) {  
						$bprice = apply_filters( 'wcpb_bundle_product_price', wc_get_price_including_tax($bundle, array("qty" => $value["quantity"])), $bundle, $value["quantity"] );												
					} else {   
						$bprice = apply_filters( 'wcpb_bundle_product_price', wc_get_price_excluding_tax($bundle, array("qty" => $value["quantity"])), $bundle, $value["quantity"] );
					}	
					if (!is_nan($bprice)) {
						$price += $bprice;
					}				
